...
function DecouvertCard({color,image,title,border}: Props) {
  return (
    <div className={`relative w-[300px] h-[200px] overflow-auto rounded-lg border-2 ${border}`}>
        ...
        <h1 className={`z-20 absolute ${color} top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-4xl font-semibold uppercase`}>{title}</h1>
    </div>
  )
}
